...
- Rewrote five tests that were written against POSIX-only assumptions so they hold on every platform: `npm-ci-guard.test.js`'s four real-executable-proof tests now invoke the platform's real npm launcher (`npm.cmd` on Windows) directly with no shell fallback, and `tools.test.ts`'s relative-PATH test now reaches a real platform-native executable fixture through a genuinely relative `PATH` entry instead of a POSIX shebang script. Same predicates as before, now provable on every platform they run on.
