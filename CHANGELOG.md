...
- Removed a temporary Windows diagnostic workflow: its own known-broken control never actually exercised the failure mode it was meant to detect, so a run of it couldn't be trusted as evidence of anything. The real CI gate reproduces the underlying Windows test failures on its own.
