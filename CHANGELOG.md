...
- Removed a temporary Windows diagnostic workflow. Its known-broken control was meant to prove the workflow could detect a real hang, but the hosted run's inner 30-second test timeout ended the control before the workflow's own outer 45-second observation point could see it - the workflow's stated validity precondition failed, making that run inadmissible as evidence either way. The real CI gate reproduces the underlying Windows test failures on its own.
