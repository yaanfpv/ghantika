 ...
 *     every plain-poll response must still byte-match, whether or not the
 *     Tasks adapter is actively engaged elsewhere in the same process.
 ...
 *     auto-resume; a real host's own auto-resume is a separate,
 *     disclosed-pending verification, gated on an actual such host
 *     existing to test against - tracked outside this repo).
 *
...
test("run()'s CreateTaskResult response is observably received before any tasks/get can observe buffered output (real timing over the real adapter path, never a pre-seeded order), and run() does not block on an earlier still-running task", async () => {
  ...
  try {
    ...
    // A real handler on the real client, recording the exact moment a wake
    // notification's own handler actually runs.
    pair.client.setNotificationHandler(
      TASKS_STATUS_NOTIFICATION_METHOD,
      { params: passthroughSchema() },
      () => {
        orderLog.push({ type: "wake", order: orderCounter });
        orderCounter += 1;
      }
    );

    // PART A - ordering: a command that writes output IMMEDIATELY and then
    // exits quickly - a genuine race between "the run() response arriving"
    // and "the job's own output becoming observable via a terminal,
    // output-bearing tasks/get read", never an artificially delayed
    // command that would make the ordering trivial by construction.
    ...
    orderLog.push({ type: "run-response", order: orderCounter });
    orderCounter += 1;
    orderJobId = minted.taskId as string;
    assert.equal(typeof orderJobId, "string");

    const runResponseOrder = orderLog.find((entry) => entry.type === "run-response")!.order;
    ...
    for (let attempt = 0; attempt < 200; attempt += 1) {
      ...
      if (taskGet.output !== undefined) {
        firstOutputBearingOrder = orderCounter;
        orderCounter += 1;
        break;
      }
    ...
    // Every wake observed (if any fired at all, for this job's short life)
    // must also have arrived strictly AFTER the run() response - proving
    // the commit is never preceded by any wake-carried signal either.
    for (const wakeEntry of orderLog.filter((entry) => entry.type === "wake")) {
      assert.ok(
...
test("green control: a bounded stream UNDER FIREHOSE_LINES_PER_SEC runs to its normal terminal completely UNTOUCHED - the watch is never auto-stopped", async () => {
  ...
  try {
    ...
    // test/tasks-lifecycle.test.ts's own green control already uses.
    mock.timers.enable({ apis: ["setTimeout", "Date"], now: Date.now() });
    try {
    ...
    // Runs to its normal (here: killed) terminal, completely untouched by
    // any firehose guard.
...
// =============================================================================
// Strictly additive: byte-identical plain poll against a frozen golden.
// The golden was captured from a REAL spawned build of the commit right
// before the wake layer merged (f56dfd7), speaking real, raw JSON-RPC
// over its real stdin/stdout to a plain (non-Tasks-capable) connection - see
// test/fixtures/plain-poll-golden.json's own header. It is LOADED here, not
// regenerated from the currently-running server.
// =============================================================================
...
// identity mismatch. None of the four is something this story's wake layer
...
const RACY_CONFIRMATION_FIELDS = new Set([
  "kill_confirmed",
  "identity_confirmed",
  "identity_capture",
 ...
 * process) rather than reusing the golden's exact literal string.
 */
function canonicalizePlainPollResponse(value: unknown): unknown {
  ...
  if (value !== null && typeof value === "object") {
    ...
    for (const [key, v] of Object.entries(value as Record<string, unknown>)) {
      ...
      if (RACY_CONFIRMATION_FIELDS.has(key)) continue;
...
test("all six tools' plain-poll responses stay byte-identical (canonical projection) to the frozen pre-wake golden, with no OTHER connection ever engaging Tasks capability", async () => {
  ...
  try {
    ...
    assert.deepEqual(canonicalizePlainPollResponse(live), golden);
  } finally {
    await pair.close();
  }
});
...
test(
  "session EOF/shutdown finitely reaps a genuinely-live task's real backing job within a bounded deadline (a hang is a distinct failure from a skip) - the real process is confirmed dead via the reused pgrep oracle, and an already-terminal task alongside it is left alone",
  ...
  async () => {
    ...
    for (let attempt = 0; attempt < 100 && terminalStatus !== "completed"; attempt += 1) {
      server.send({
        jsonrpc: "2.0",
        id: 200 + attempt,
        method: "tasks/get",
      ...
      terminalStatus = (line.parsed as TasksGetResponseBody).result?.status as string | undefined;
      if (terminalStatus !== "completed") await new Promise((resolve) => setTimeout(resolve, 20));
    }
    assert.equal(
      terminalStatus,
      "completed",
    ...
    // Trigger the real shutdown path (stdin EOF), bounded: a genuine hang
    // must fail FAST with a clear message, never merely wait out the test
    // runner's own much longer default timeout - that distinction (a hang
    // vs a skip) is exactly what this race makes observable.
    ...
    // The task record itself is confirmed gone too - not by querying it
    // (the server PROCESS that held jobStore, in memory, no longer
    // exists to ask), but by the definitional fact that the process
    // holding it has now exited cleanly: there is no separate durable
    // store it could have survived in (see README.md's documented
    // process-lifetime-only fact) - its process's own clean exit IS the
    // whole proof.
    ...
    // never caused shutdown to hang or error.
  }
);
