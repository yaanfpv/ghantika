...
// over the real MCP wire via a spawned server subprocess, EXCEPT the three
// "Windows mapping" tests at the very end - see
// test/helpers/requireSpawnPolicy.ts for what this checks and why. Those
// three spawn nothing at all: two are pure source-text checks against
// src/tools/kill.ts and src/server.ts, and the third exercises jobStore's
// own platform-independent bookkeeping directly (imported from
// dist/jobStore.js), never calling the `run` tool or a spawned server - a
// file-level before() here would fail those three too, under a policy
// they never need. So the guard is scoped to a single describe() wrapping
// just the run-dispatching tests below: node:test scopes a describe-level
// before() to only the tests nested inside it, so a thrown preflight
// failure there fails just those run-dispatching tests with the guard's
// real, actionable message, never the three Windows-mapping tests.
