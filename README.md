...
Each entry can be a bare name (resolved via ghantika's own `PATH`) or a path; either way, the entry and the command actually being run are both resolved to their real, symlink-free path before comparison, so any way of spelling the same binary reaches the same decision. `shell: true` is checked the same way, against the actual platform shell it invokes (`/bin/sh` on POSIX), not against the shell command line itself - so a shell command only ever runs once you've allowlisted the shell. A denied command comes back as a failed job with a diagnostic saying so, rather than running or throwing. This is server configuration, not something a tool call can supply or widen: nothing in `command`'s own arguments is ever read as policy, and a missing or malformed policy file denies everything rather than falling open.
