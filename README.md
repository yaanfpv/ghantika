...
The wake fires in response to a job this same session started, on two triggers: the job reaching a terminal state, or the job producing output while it is still running - never unconditionally, never on a plain timer, never for a job it did not start. See "Output-triggered wakes, and their fixed batching window" below for how the second trigger coalesces a stream of output into individual wakes.
...
### Output-triggered wakes, and their fixed batching window

Both the inherited-messaging wake above and the app-server wake below fire on two triggers: a job reaching a terminal state, and a job producing output while it is still running. A terminal-only wake is structurally unreachable for a job that is designed never to terminate - a long-lived watcher started with `run` and left running for the life of a session, the doorbell shape this project's own inbox-notification pattern depends on. Without an output trigger, that shape would be woken exactly zero times for its whole life.

The two triggers are independent and additive - the terminal wake is unchanged, this only adds the second one alongside it - and both read the same two gates (`GHANTIKA_DISABLE_CLAUDE_MESSAGING_WAKE`, `GHANTIKA_WAKE_TRANSPORT_ENABLED`) with the identical meaning described in their own sections; neither trigger changes what an operator opted into, only when a wake this project already sends can fire.

**A fixed window, not a rolling one.** Output arriving within about 200ms of the first line in a batch collapses into one wake - the same coalescing window the existing Tasks-extension notification already uses. The window is fixed rather than rolling: it opens on the first line and every later line arriving before it closes joins that same window rather than resetting it, so a job producing output continuously keeps flushing on successive windows throughout its run, rather than holding everything back until it goes quiet. Every distinct burst of output arriving outside an already-open window opens its own new one and produces its own separate wake - there is no rate limit above the window. A window still open when the job reaches a terminal state is cancelled and never fires; the job's separate terminal wake fires instead, and any output the cancelled window was still holding remains readable through `output`. The only bound this implies on the output-triggered wake's own rate is the window's own width: at most one wake per window, so at most five per second for a job that never stops talking.

**A disclosed limit, not a bug: a short line with no trailing newline produces no output-triggered wake until the job's stream ends.** ghantika only materializes a line - and only a materialized line can trigger this wake - on a real newline, a forced cut once a single line grows past this project's own per-line size cap, or the stream finalizing at process exit. A watcher that emits genuine progress with no trailing newline, and stays under that size cap, produces nothing this wake can see until the process it is watching actually exits. The terminal-state wake still covers that job's eventual completion regardless; what is not covered is a mid-life nudge for output shaped this way. `status`/`output`/`tail` remain the honest way to read it in the meantime, exactly as they cover every other case this wake might miss.

### The app-server wake (Codex)

Independent of both the Tasks-extension notification described in the "Early days" note above and the Claude Code mechanism above, ghantika also carries its own code that pushes a message directly into a Codex thread once a job it started finishes, or produces output while still running - not a client-side mechanism, and not gated on any capability negotiation.
...
When a `run` request's own metadata carries a non-empty raw thread ID, ghantika attempts a wake over an ordered pair of transports - the app-server protocol Codex's own tooling uses first, then the desktop app's IPC socket - addressed to whatever that ID names, on either of the two triggers described in "Output-triggered wakes, and their fixed batching window" above: the job reaching a terminal state, or the job producing output while still running (subject to that section's own fixed 200ms batching window). A refused, unavailable, or thrown attempt on one transport falls through to the next rather than stopping there; only once every configured transport has been tried and none delivered does the final, exhausted outcome get logged. An attempt is not the same as a delivery, and the ID's presence doesn't by itself prove it names the job's own originating thread - only what this codebase's own wake-target design assigns to it.
