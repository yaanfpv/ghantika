...
A job's output is captured as it happens, stdout and stderr tracked separately, so `output` and `tail` read what the process has actually written rather than a snapshot from whenever you happened to ask. Each stream keeps a bounded window of that: the most recent 10,000 lines or 1 MiB, whichever it hits first, with one deliberate exception, a single oversized line is kept whole even when that one entry alone runs past the byte bound. Once a stream starts dropping its oldest lines to stay inside the window, the response says so with a gap marker. Read a single stream and that marker is the exact range that went missing. Read the default merged view of both streams and it's a best-effort marker instead, since nothing records the true interleaving of two streams and inventing one would be worse than admitting it. Either way the recent history is real and the old history is reported as gone, never quietly missing.
...
Those `seq` values are the real per-line numbers because the call asked for one stream. Leave `stream` out and you get the merged default, which numbers events on its own interleave axis instead, so don't read a merged `seq` as a line's true position in either stream. A response that had to drop old history carries `"truncated": true` and puts a `{ "gap": [start, end] }` marker at the front of `events`. Feed `next_cursor` back as `output`'s `after_cursor` to read only what's new since last time.
...
| `env`     | object             | no       | `{ "mode": "merge" \| "replace", "vars": { ... } }`. `vars` maps variable names to values. `merge` (the default) layers `vars` over a minimal base of `PATH` and `HOME`, or `PATH`, `SystemRoot` and `USERPROFILE` on Windows; `replace` passes `vars` alone with no base at all - true without exception on POSIX, but not quite the whole story on Windows (see Platform notes). |
...
`env.mode: "replace"` is not quite the same guarantee on Windows as it is on POSIX, and the table above says so rather than leaving it implied. On POSIX, `replace` really does mean your vars and nothing else - the platform's own process-spawning code just passes whatever environment array it's given straight through, with no additions of its own. On Windows, Node's underlying process-spawning library (libuv) checks each of a fixed set of Windows-essential variables - `HOMEDRIVE`, `HOMEPATH`, `LOGONSERVER`, `PATH`, `SYSTEMDRIVE`, `SYSTEMROOT`, `TEMP`, `USERDOMAIN`, `USERNAME`, `USERPROFILE`, and `WINDIR` - against the env block it's given: if you already supplied one of these names yourself, your value is kept exactly as given; for any of them you did NOT supply, libuv fills it in from the machine running ghantika, but only if that machine actually has it set. None of that depends on `env.mode` - the same fill-in-if-missing, keep-if-supplied behavior applies either way, because some of Windows's own machinery (Winsock's init, among other things) won't start correctly without them. Ghantika doesn't add any of these itself, and there's no reasonable way to strip an already-injected variable back out of a process once it exists. It's a small, fixed, named list, not an open-ended "Windows does whatever it wants" - but on that one platform, `replace` means "your vars, including any of that list you supplied yourself, plus whichever of the rest the machine has."
