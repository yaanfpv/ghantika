...
A job also outlives any single check. Start it, go do something else, ask again on a much later tool call, and `status` still tells you exactly what happened, for as long as the same ghantika server process is up. Closing the MCP session is where that ends: stdin EOF reaps every live job before the server exits, on every platform. A termination signal does the same on POSIX; on Windows it doesn't, and any job still running at that moment is left behind (see Platform notes below). Nothing about a job survives a restart either way, since the store is in memory and deliberately so.
...
**Windows shutdown behavior.** Ghantika reaps every job it still has running before it exits, but on Windows that only happens when the client closes ghantika's stdin. Windows has no real signal delivery: an external kill (Task Manager, `taskkill`, or a host's own process-management code ending the ghantika subprocess) calls `TerminateProcess`, which ends the process immediately with no way for it to run any of its own shutdown code, on any platform, by design. So on Windows, a host that stops ghantika by any means other than closing its stdin doesn't give it the chance to clean up, and any job still running at that moment is left behind as an orphaned process. If you're building a host that runs ghantika on Windows and you want its jobs cleaned up on exit, close its stdin rather than killing it.
