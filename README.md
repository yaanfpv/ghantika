...
> 🚧 **Early days.** This is a young project moving fast. The background command runner and all six tools (run, status, list, output, tail, kill) are real and tested today, driven by polling. The Tasks-extension wake this README also describes is real too, not just on the way: a capable client gets pushed a notification as a job's output arrives, coalesced into one notification per short window rather than one per arrival, on top of the same poll floor, which stays authoritative either way. The terminal transition itself fires no notification - a client learns a job is done through the ordinary poll surface, same as any other status change. This has been proven against a simulated Tasks-capable client; whether a real, separately-authored MCP host resumes automatically on that notification is disclosed as pending, verifiable only once such a host exists to test against. Treat anything else you hit that feels rough as exactly that, not a hidden gap.
...
**Set it up once, use it everywhere.** It's a standard [MCP](https://modelcontextprotocol.io) server over stdio, so every client that speaks MCP can start jobs and read them back with the same six tools, by polling. A client that also speaks the MCP Tasks extension gets more than that: ghantika wakes it directly as new output arrives, instead of it having to ask. Every client reads the same jobs the same way, and the job runs regardless of whether anything is watching.
...
- A fixed sleep-and-recheck loop burns a full round trip on every guess at how long something takes. Ghantika answers on the job's real state, not on a timer you had to estimate up front, and a Tasks-capable client gets rung as new output arrives instead of having to ask.
