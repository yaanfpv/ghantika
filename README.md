...
### Output-triggered wakes, and their cooldown
...
**A repeat gate, distinct from the burst-collapse window.** A tight burst of output lines arriving within about 200ms of each other still collapses into one wake, exactly like the existing Tasks-extension notification's own coalescing window. On top of that, once an output-triggered wake fires for a job, no further output-triggered wake for that same job can fire for two minutes - a job producing output continuously injects at most one wake every two minutes, never one per line and never one every 200ms. Output arriving during that two-minute window is never dropped: it is tracked, and fires exactly once, the moment the window closes, covering everything that arrived while it was open. This exists because the underlying wake, on every transport this project uses, injects a real turn into the receiving session unconditionally - there is no idle check anywhere in the chain that would make a wake cheap to repeat, so repeating it is capped explicitly instead. For the shape this feature exists for - a doorbell whose real events are minutes to hours apart - the cooldown never engages at all; it exists for the opposite shape, a job that never stops producing output.
