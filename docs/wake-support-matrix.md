...
- `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS` - a duration in milliseconds. Set it to a value greater than zero and below the client's own MCP call timeout (`MCP_TOOL_TIMEOUT`, 60000ms unless you've changed it). In an **interactive** session this one variable is enough on its own.
...
**Observed:** 2026-08-12, ~03:51-03:52 UTC, via ghantika's own `follow` tool against a live Claude Code session, with `CLAUDE_CODE_MCP_AUTO_BACKGROUND_MS=1000` and `CLAUDE_AUTO_BACKGROUND_TASKS=1` both set. The client build in use was not captured directly at observation time; based on install timestamps on the same machine, the most likely build at that time was Claude Code 2.1.226.
